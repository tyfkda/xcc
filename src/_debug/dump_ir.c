...
static void dump_ir(FILE *fp, IR *ir) {
  ...
  switch (ir->kind) {
  case IR_BOFS:   FPRINTF(fp, "%R = &[rbp %c %d]\n", ir->dst, ir->bofs.frameinfo->offset >= 0 ? '+' : '-', ir->bofs.frameinfo->offset > 0 ? ir->bofs.frameinfo->offset : -ir->bofs.frameinfo->offset); break;
  case IR_IOFS:   FPRINTF(fp, "%R = &%.*s\n", ir->dst, NAMES(ir->iofs.label)); break;
