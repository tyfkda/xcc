...
static inline ArgInfo *collect_funargs(Expr *expr, FuncallWork *work) {
  Expr *func = expr->funcall.func;
  const Type *functype = get_callee_type(func->type);
  VarInfo *ret_varinfo = work->ret_varinfo;
  ...
  ArgInfo *arg_infos = calloc_or_die(sizeof(*arg_infos) * arg_count);
...
static inline void gen_funargs(Expr *expr, FuncallWork *work) {
  ...
  ArgInfo *arg_infos = collect_funargs(expr, work);
