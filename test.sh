...
try 55 'int i; int acc; i = acc = 0; while (i <= 10) { acc = acc + i; i = i + 1; } return acc;'
try 55 'int i; int acc; for (i = acc = 0; i <= 10; i = i + 1) acc = acc + i; return acc;'
try 11 'int *p; int x; x = 10; p = &x; *p = *p + 1; return x;'
try 123 'int a[3]; int *p; p = a; p = p + 1; *p = 123; return *(a + 1);'
